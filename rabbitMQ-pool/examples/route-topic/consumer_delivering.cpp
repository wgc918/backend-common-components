...
int main()
{
    ...
    // 创建临时队列并绑定到 "*.*.delivering"（所有派送中的消息）
    ...
    q.bind(*pool.acquire(), "topic_logistics", "*.*.delivering");
    ...
    consumer.set_queue(q.name())
        .on_message(
            [](const amqp_envelope_t& envelope)
            {
                ...
                std::cout << "[Delivering] recv: " << body << std::endl;
            })
        .on_error([](const std::string& str) { std::cout << "recv error: " << str << std::endl; });
    consumer.start();
    std::cout << "Delivering Consumer [" << q.name() << "] started. (Ctrl+C to stop)" << std::endl;
